<?php 
	...
	if(!empty($_GET['del'])){
		$id=$_GET['del'];
		mysql_query("DELETE FROM TALLERISTAS WHERE id='$id'");
		header('index.php');
	}
?>
											...
											<div class="modal-dialog">
												<div class="modal-content">
													<div class="modal-header">
														...
														<h3 align="center" class="modal-title" id="myModalLabel">Nuevo Tallerista</h3>
													</div>
										<div class="panel-body">
										<div class="row">
										 <ul class="nav nav-tabs nav-justified">
											...
											<div class="col-md-12">
											<br>			
											<input class="form-control" name="codigo" placeholder="Cédula"  autocomplete="off" required><br>											
											...
											<input class="form-control" title="Se necesita un salario" name="salario" placeholder="Salario"  autocomplete="off" required><br>											
											</div>
											<div class="col-md-6">
												...
												<select class="form-control" name="responsabilidad" autocomplete="off" required>																					
												<option value="" selected disabled>--Responsabilidad--</option>
													<?php
															$p=mysql_query("SELECT * FROM responsabilidad WHERE estado='s'");				
															while($r=mysql_fetch_array($p)){
																echo '<option value="'.$r['id'].'">'.$r['nombre'].'</option>';
															}
														?>												
												</select><br> 
												<select class="form-control" name="ttallerista" autocomplete="off" required>																					
												<option value="" selected disabled>--Tipo Tallerista--</option>
													<?php
															$p=mysql_query("SELECT * FROM ttalleristas WHERE estado='s'");				
															while($r=mysql_fetch_array($p)){
																echo '<option value="'.$r['id'].'">'.$r['nombre'].'</option>';
															}
														?>												
                            ...
                            <div class="table-responsive">
								<?php 
									if(!empty($_POST['nombre'])){ 
										...
										$responsabilidad=limpiar($_POST['responsabilidad']);
										$ttallerista=limpiar($_POST['ttallerista']);
										$estado=limpiar($_POST['estado']);
			
										
										if(empty($_POST['id'])){
											$oPaciente=new Proceso_Paciente('',$codigo,$nombre,$salario,$responsabilidad,$ttallerista,$estado);
											$oPaciente->crear();
											echo mensajes('Tallerista "'.$nombre.'" Creado con Exito','verde');
										}else{
											$id=limpiar($_POST['id']);
											$oPaciente=new Proceso_Paciente($id,$codigo,$nombre,$salario,$responsabilidad,$ttallerista,$estado);
											$oPaciente->actualizar();
											echo mensajes('Tallerista "'.$nombre.'" Actualizado con Exito','verde');
										}
									}
                                    ...
                                    <tbody>
										<?php 
											if(!empty($_POST['buscar'])){
												...
												$pame=mysql_query("SELECT * FROM talleristas WHERE nombre LIKE '%$buscar%' ORDER BY nombre");	
											}else{
												$pame=mysql_query("SELECT id,codigo,nombre,salario,estado FROM talleristas  ORDER BY nombre");		
											}		
                                        ...
                                        <tr class="odd gradeX">
                                            ...
                                            <td><i class="fa fa-user fa-2x"></i> <?php echo $row['salario']; ?></td>	
										...
										<div class="row">
											...
											<div class="input-group">
												  ...
												  <input class="form-control" title="Se necesita un salario"  name="salario" placeholder="salario" value="<?php echo $row['salario']; ?>" autocomplete="off" required><br>											
											</div><br>	
											</div><br>											
											</div>
											...
											<div class="col-md-6">
											<div class="input-group">
												  <span class="input-group-addon">responsabilidad:</span>
												 <select class="form-control" name="responsabilidad" onchange="pais(this.value);" autocomplete="off" required>			
													<?php
														$p=mysql_query("SELECT * FROM responsabilidad WHERE estado='s'");				
														while($r=mysql_fetch_array($p)){
															if($r['id']==$row['responsabilidad']){
											...
											<div class="input-group">
												  <span class="input-group-addon">Tipo Tallerista:</span>
												 <select class="form-control" name="ttallerista" onchange="pais(this.value);" autocomplete="off" required>			
													<?php
														$p=mysql_query("SELECT * FROM ttalleristas WHERE estado='s'");				
														while($r=mysql_fetch_array($p)){
															if($r['id']==$row['ttallerista']){
																echo '<option value="'.$r['id'].'" selected>'.$r['nombre'].'</option>';
															}else{
																echo '<option value="'.$r['id'].'">'.$r['nombre'].'</option>';
															}
														...
														<div class="modal-content">
															...
															<div class="row" align="center">                                       
																...
																<div class="alert alert-danger">
																	...
																	una vez Eliminado el Tallerista [ <?php echo $row['nombre']; ?> ]<br> 
