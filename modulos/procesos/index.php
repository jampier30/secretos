<?php 
	...
	if(!empty($_GET['del'])){
		$id=$_GET['del'];
		mysql_query("DELETE FROM procesos WHERE id='$id'");
		header('index.php');
	}
?>
                                    ...
                                    <tbody>
										<?php 
											if(!empty($_POST['buscar'])){
												...
												$pame=mysql_query("SELECT * FROM procesos WHERE nombre LIKE '%$buscar%' ORDER BY nombre");	
											}else{
												$pame=mysql_query("SELECT id,codigo,nombre,estado FROM procesos  ORDER BY nombre");		
											}		
